...
Brickout.graphics = function () {
  ...
  function drawImage(spec) {
    ...
    context.translate(-spec.center.x, -spec.center.y);
    context.drawImage(spec.image, spec.center.x - spec.size / 2, spec.center.y - spec.size / 2, spec.size, spec.size);
    context.restore();
  }
...
function ParticleSystem(graphics, image1) {
  ...
  that.createEffect = function (spec) {
    var x,
        ...
        deltaY = (spec.bottom - spec.top) / 10,
        // bottom - top : remember that Y is position down the screen
    posX,
        posY;
    spec.speed = {
      mean: 20,
  ...
  function makeParticle(spec, x, y) {
    var p = {
      ...
      // pixels per second
      rotation: 0,
      lifetime: Math.abs(Random.nextGaussian(spec.lifetime.mean, spec.lifetime.stdev)),
...
var particleSystem1 = ParticleSystem(Brickout.graphics, GameState.assets["smoke"]);
var particleSystem2 = ParticleSystem(Brickout.graphics, GameState.assets["fire"]);
...
//# sourceMappingURL=particle-system.dev.js.map
